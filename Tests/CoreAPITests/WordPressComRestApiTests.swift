...
class WordPressComRestApiTests: XCTestCase {
    ...
    private func isRestAPIRequest() -> HTTPStubsTestBlock {
        return { request in
            ...
            let result =
            expectedScheme == actualScheme && expectedHost == actualHost && expectedPath == actualPath
            return result
        }
    }
    ...
    func testSuccessfullCall() throws  {
        ...
        let api = WordPressComRestApi(oAuthToken: "fakeToken")
        api.GET(
            wordPressMediaRoutePath, parameters: nil,
    ...
    func testBaseUrl() async throws {
        ...
        try XCTAssertTrue(
            XCTUnwrap(request?.url?.absoluteString).hasPrefix("https://public-api.wordpress.com/path"))
        ...
        try XCTAssertTrue(
            XCTUnwrap(request?.url?.absoluteString).hasPrefix("http://localhost:8080/local"))
    }
    ...
    func testInvalidTokenFailedCall() throws {
        ...
        api.GET(
            ...
            failure: { (error, _) in
                expect.fulfill()
                XCTAssert(
                    error.domain == "WordPressKit.WordPressComRestApiError",
                    "The error should a WordPressComRestApiError")
                XCTAssert(
                    error.code == Int(WordPressComRestApiErrorCode.invalidToken.rawValue),
                    "The error code should be invalid token")
            })
        self.waitForExpectations(timeout: 2, handler: nil)
    }
        ...
        api.GET(
            ...
            failure: { (error, _) in
                ...
                XCTAssert(
                    ...
                    "The error domain should be WordPressComRestApiErrorDomain")
                XCTAssert(
                    error.code == Int(WordPressComRestApiErrorCode.responseSerializationFailed.rawValue),
                    "The code should be invalid response serialization")
            })
        self.waitForExpectations(timeout: 2, handler: nil)
    }
    ...
    func testInvalidJSONSentFailedCall() throws {
        ...
        let api = WordPressComRestApi(oAuthToken: "fakeToken")
        api.POST(
            wordPressMediaNewEndpointPath, parameters: nil,
            success: { (_: AnyObject, _: HTTPURLResponse?) in
                expect.fulfill()
                XCTFail("This call should fail")
            },
            failure: { (error, _) in
                ...
                XCTAssert(
                    error.code == Int(WordPressComRestApiErrorCode.invalidInput.rawValue),
                    "The error code should be invalid input")
            })
        self.waitForExpectations(timeout: 2, handler: nil)
    }
        ...
        api.POST(
            ...
            failure: { (error, _) in
                ...
                XCTAssert(
                    ...
                    "The error domain should be WordPressComRestApiError")
                XCTAssert(
                    error.code == Int(WordPressComRestApiErrorCode.authorizationRequired.rawValue),
                    "The error code should be AuthorizationRequired")
            })
        self.waitForExpectations(timeout: 2, handler: nil)
    }
        ...
        api.POST(
            ...
            failure: { (error, _) in
                ...
                XCTAssert(
                    ...
                    "The error domain should be WordPressComRestApiError")
                XCTAssert(
                    error.code == Int(WordPressComRestApiErrorCode.uploadFailed.rawValue),
                    "The error code should be AuthorizationRequired")
            })
        self.waitForExpectations(timeout: 2, handler: nil)
    }
    ...
    func testMultipleErrorsFailedMultiPartPostCall() throws {
        ...
        let api = WordPressComRestApi(oAuthToken: "fakeToken")
        api.multipartPOST(
            wordPressMediaNewEndpointPath, parameters: nil, fileParts: [],
            success: { (_: AnyObject, _: HTTPURLResponse?) in
                expect.fulfill()
                XCTFail("This call should fail")
            },
    ...
    func testStreamMethodCallWithInvalidFile() throws {
        ...
        let filePart = FilePart(
            parameterName: "file", url: URL(fileURLWithPath: "/a.txt") as URL, filename: "a.txt",
            mimeType: "image/jpeg")
        api.multipartPOST(
            wordPressMediaNewEndpointPath, parameters: nil, fileParts: [filePart],
            success: { (_: AnyObject, _: HTTPURLResponse?) in
                expect.fulfill()
                XCTFail("This call should fail")
            },
    ...
    func testStreamMethodParallelCalls() throws {
        ...
        let api = WordPressComRestApi(oAuthToken: "fakeToken")
        let filePart = FilePart(
            parameterName: "media[]", url: mediaURL as URL, filename: "test-image.jpg",
            mimeType: "image/jpeg")
        let progress1 = api.multipartPOST(
            wordPressMediaNewEndpointPath, parameters: nil, fileParts: [filePart],
            success: { (_: AnyObject, _: HTTPURLResponse?) in
                XCTFail("This call should fail")
            },
    ...
    func testCancelationOfRequest() {
        ...
        api.POST(
            ...
            failure: { (error, _) in
                expect.fulfill()
                XCTAssertEqual(
                    error.domain, NSURLErrorDomain, "The error domain should be NSURLErrorDomain")
                XCTAssertEqual(
                    error.code, NSURLErrorCancelled, "The error code should be NSURLErrorCancelled")
            })
        self.waitForExpectations(timeout: 2, handler: nil)
    }
    ...
    func testTooManyRequestError() throws {
        ...
        api.GET(
            ...
            failure: { error, _ in
                ...
                XCTAssertEqual(
                    error.userInfo[WordPressComRestApi.ErrorKeyErrorCode] as? String, "too_many_requests")
    ...
    func testPostParametersContent() throws {
        ...
        XCTAssertEqual(
            request.url?.absoluteString, "https://public-api.wordpress.com/rest/v1/foo?locale=en")
