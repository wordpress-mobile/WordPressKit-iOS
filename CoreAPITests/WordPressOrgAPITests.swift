...
class WordPressOrgAPITests: XCTestCase {
  ...
  func testSelfHostedSiteSimpleGet() async {
    ...
    stub(
      condition: isMethodGET()
        && isAbsoluteURLString("https://wordpress.org/wp-json/wp/v2/hello-world?foo=bar")
    ) { _ in
      expectation.fulfill()
      return HTTPStubsResponse(data: Data(), statusCode: 200, headers: nil)
    }
    let api = WordPressOrgRestApi(
      selfHostedSiteWPJSONURL: URL(string: "https://wordpress.org/wp-json/")!,
      credential: fakeCredential)
    let _ = await api.get(
      path: "wp/v2/hello-world", parameters: ["foo": "bar"], type: AnyResponse.self)
  ...
  func testSelfHostedSiteSimplePost() async {
    ...
    stub(
      condition: isMethodPOST()
        && isAbsoluteURLString("https://wordpress.org/wp-json/wp/v2/hello-world")
    ) { request in
  ...
  func testWPComSiteSimpleGet() async {
    ...
    stub(
      condition: isMethodGET()
        && isAbsoluteURLString(
          "https://public-api.wordpress.com/wp/v2/sites/42/hello-world?foo=bar")
    ) { request in
  ...
  func testWPComSiteSimplePost() async {
    ...
    stub(
      condition: isMethodPOST()
        && isAbsoluteURLString("https://public-api.wordpress.com/wp/v2/sites/42/hello-world")
    ) { request in
      XCTAssertEqual(request.value(forHTTPHeaderField: "Authorization"), "Bearer faketoken")
      XCTAssertEqual(request.httpBodyText, "foo=bar")
      expectation.fulfill()
  ...
  func testUserAgent() async {
    ...
    let selfHostedSite = WordPressOrgRestApi(
      selfHostedSiteWPJSONURL: URL(string: "https://wordpress.org/wp-json/")!,
      credential: fakeCredential, userAgent: "fake-user-agent")
    let _ = await selfHostedSite.get(path: "/wp/v2/hello-world", type: AnyResponse.self)

    let dotComSite = WordPressOrgRestApi(
      dotComSiteID: 42, bearerToken: "faketoken", userAgent: "fake-user-agent")
  ...
  func testSelfHostedSiteAuthentication() async throws {
    ...
    let unauthenticatedReuqest = expectation(
      description: "Call #1: call an endpoint with a unauthenticated request")
    let ajaxNonceRequest = expectation(description: "Call #2: call ajax endpoint to get nonce")
    let authenticatedReuqest = expectation(
      description: "Call #3: call an endpoint with an authenticated request")
    ...
    await fulfillment(
      of: [unauthenticatedReuqest, ajaxNonceRequest, authenticatedReuqest], timeout: 0.1,
      enforceOrder: true)

    // Remove added stubs and add a new one.
    let requestsHasNonce = expectation(
      description: "Call an endpoint with an authenticated request")
  ...
  func testSelfHostedSiteAuthenticationFailure() async throws {
    ...
    let loginRequest = expectation(
      description: "Authenticate during fetching the ajax and new post webpage")
    loginRequest.expectedFulfillmentCount = 2
    let authenticatedReuqest = expectation(
      description: "[Should never happen] call an endpoint with an authenticated request")
    ...
    await fulfillment(
      of: [unauthenticatedReuqest, ajaxNonceRequest, newPostWebpageRequest, loginRequest],
