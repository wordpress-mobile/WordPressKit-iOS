...
class WordPressOrgRestApiTests: XCTestCase {
  ...
  func testSuccessfulPostCall() async throws {
    ...
    let response = try await api.post(
      path: "wp/v2/blocks/6", parameters: parameters, type: Response.self
    ).get()
  ...
  func testRequestPathModificationsWPV2() async throws {
    stub(condition: isPath("/wp/v2/sites/1001/themes") && containsQueryParams(["status": "active"]))
    { _ in
    ...
    let _ = try await api.get(
      path: "/wp/v2/themes", parameters: ["status": "active"], type: AnyResponse.self
    ).get()
  }
  ...
  func testSettingWPComAPIURL() async {
    ...
    let api = WordPressOrgRestApi(
      dotComSiteID: 1001, bearerToken: "token", apiURL: URL(string: "http://localhost:8000")!)
  ...
  func testPathWithQuery() async {
    ...
    XCTAssertEqual(
      ...
      "https://public-api.wordpress.com/wp/v2/sites/1001/get-decodable?context=mobile")

    let _ = await api.post(path: "/wp/v2/post-decodable?context=mobile", type: AnyResponse.self)
    XCTAssertEqual(
      request?.url?.absoluteString,
      "https://public-api.wordpress.com/wp/v2/sites/1001/post-decodable?context=mobile")

    let _ = await api.get(path: "/wp/v2/get-any-json?context=mobile")
    XCTAssertEqual(
      request?.url?.absoluteString,
      "https://public-api.wordpress.com/wp/v2/sites/1001/get-any-json?context=mobile")

    let _ = await api.post(path: "/wp/v2/post-any-json?context=mobile")
    XCTAssertEqual(
      request?.url?.absoluteString,
      "https://public-api.wordpress.com/wp/v2/sites/1001/post-any-json?context=mobile")
  }
}
...
extension WordPressOrgRestApi {
  convenience init(apiBase: URL) {
    self.init(
      ...
      credential: .init(
        loginURL: URL(string: "https://not-used.com")!, username: "user", password: "pass",
        adminURL: URL(string: "https://not-used.com")!)
    )
  }
}
