...
final class WordPressOrgXMLRPCValidatorTests: XCTestCase {
    ...
    func testNotWordPressSiteError() throws {
        ...
        let validator = WordPressOrgXMLRPCValidator()
        validator.guessXMLRPCURLForSite(
            "https://www.apple.com/", userAgent: "test/1.0",
            success: {
            ...
            XCTAssertTrue(
                validatorError == .invalid || validatorError == .notWordPressError, "Got an error: \(error)"
            )
            failure.fulfill()
        }
    ...
    func testSuccessWithSiteAddress() throws {
        ...
        let validator = WordPressOrgXMLRPCValidator()
        validator.guessXMLRPCURLForSite(
            "https://www.apple.com/blog", userAgent: "test/1.0",
            success: {
    ...
    func testManyRedirectsError() throws {
        ...
        for number in 1...30 {
            stub(condition: isMethodPOST() && { $0.url!.path.hasPrefix("/redirect/\(number)-req") }) {
                HTTPStubsResponse(
                    data: Data(), statusCode: 302,
                    headers: [
                        "Location": $0.url!.absoluteString.replacingOccurrences(
                            of: "/redirect/\(number)-req", with: "/redirect/\(number + 1)-req")
                    ])
            }
        }
        ...
        let validator = WordPressOrgXMLRPCValidator()
        validator.guessXMLRPCURLForSite(
            "https://www.apple.com/redirect/1-req", userAgent: "test/1.0",
            success: {
                XCTFail("Unexpected result: \($0)")
            }
        ) { error in
    ...
    func testMobilePluginRedirectedError() throws {
        ...
        let validator = WordPressOrgXMLRPCValidator()
        validator.guessXMLRPCURLForSite(
            "https://www.apple.com/xmlrpc.php", userAgent: "test/1.0",
            success: {
                XCTFail("Unexpected result: \($0)")
            }
        ) { error in
    ...
    func testForbiddenError() {
        ...
        validator.guessXMLRPCURLForSite(
            ...
            XCTAssertTrue(
                validatorError == .invalid || validatorError == .forbidden, "Got an error: \(error)")
            failure.fulfill()
        }
        wait(for: [failure], timeout: 0.3)
    }
    ...
    func testXMLRPCMissingError() {
        stub(
            condition: isAbsoluteURLString("https://www.apple.com/xmlrpc.php")
            || isAbsoluteURLString("http://www.apple.com/xmlrpc.php")
        ) { _ in
            HTTPStubsResponse(data: Data(), statusCode: 403, headers: nil)
        }
        ...
        validator.guessXMLRPCURLForSite(
            ...
            XCTAssertTrue(
                validatorError == .xmlrpc_missing || validatorError == .invalid, "Got an error: \(error)")
            failure.fulfill()
        }
        wait(for: [failure], timeout: 0.3)
    }
...
extension WordPressOrgXMLRPCValidatorTests {
    fileprivate func makeUnsecuredAppTransportSecuritySettings() -> AppTransportSecuritySettings {
