...
class WordPressComOAuthClientTests: XCTestCase {
    ...
    enum OAuthURL: String {
        ...
        case socialLogin2FA =
                "https://wordpress.com/wp-login.php?action=two-step-authentication-endpoint&version=1.0"
        case socialLogin = "https://wordpress.com/wp-login.php?action=social-login-endpoint&version=1.0"
        case requestWebauthnChallenge =
                "https://wordpress.com/wp-login.php?action=webauthn-challenge-endpoint"
        case verifySignature =
                "https://wordpress.com/wp-login.php?action=webauthn-authentication-endpoint"
    }
    ...
    func testAuthenticateUsernameNo2FAFailureWrongPasswordCase() {
        ...
        client.authenticate(
            ...
            failure: { (error) in
                expect.fulfill()
                XCTAssertEqual(
                    error.authenticationFailureKind, .invalidRequest, "The code should be invalid request")
            }
        )
        waitForExpectations(timeout: 2, handler: nil)
    }
    ...
    func testAuthenticateUsername2FAWrong2FACase() {
        ...
        client.authenticate(
            ...
            failure: { (error) in
                expect.fulfill()
                XCTAssertEqual(
                    error.authenticationFailureKind, .needsMultifactorCode,
                    "The code should 'be needs multifactor'")
            }
        )
        waitForExpectations(timeout: 2, handler: nil)
    ...
    func testRequestSocial2FACodeWithUserID() {
        ...
        let expect = expectation(description: "One callback should be invoked")
        let client = WordPressComOAuthClient(clientID: "Fake", secret: "Fake")
        client.requestSocial2FACode(
            userID: 0, nonce: "nonce",
            success: { (newNonce) in
                expect.fulfill()
    ...
    func testAuthenticateWithIDToken() {
        stub(condition: isOauthTokenRequest(url: .socialLogin)) { _ in
            let stubPath = OHPathForFile(
                "WordPressComAuthenticateWithIDTokenBearerTokenSuccess.json", type(of: self))
            return fixture(
                filePath: stubPath!, headers: ["Content-Type" as NSObject: "application/json" as AnyObject])
        }
    ...
    func testAuthenticateWithIDToken2FANeeded() {
        stub(condition: isOauthTokenRequest(url: .socialLogin)) { _ in
            let stubPath = OHPathForFile(
                "WordPressComAuthenticateWithIDToken2FANeededSuccess.json", type(of: self))
            return fixture(
                filePath: stubPath!, headers: ["Content-Type" as NSObject: "application/json" as AnyObject])
        }
    ...
    func testAuthenticateWithIDTokenUserNeedsConnection() {
        stub(condition: isOauthTokenRequest(url: .socialLogin)) { _ in
            let stubPath = OHPathForFile(
                "WordPressComAuthenticateWithIDTokenExistingUserNeedsConnection.json", type(of: self))
            return fixture(
                filePath: stubPath!, status: 400,
                headers: ["Content-Type" as NSObject: "application/json" as AnyObject])
        }
    ...
    func testAuthenticateSocialLoginUser() {
        ...
        let client = WordPressComOAuthClient(clientID: "Fake", secret: "Fake")
        client.authenticate(
            socialLoginUserID: 1, authType: "authenticator", twoStepCode: "two_step_code",
            twoStepNonce: "two_step_nonce",
            success: { (token) in
                expect.fulfill()
                XCTAssert(!token!.isEmpty, "There should be a token available")
                XCTAssert(token == "bearer_token", "The newNonce should match")
    ...
    func testRequestWebauthnChallengeReturnsCompleteChallengeInfo() {
        ...
        let expect = expectation(description: "One callback should be invoked")
        let client = WordPressComOAuthClient(clientID: "Fake", secret: "Fake")
        client.requestWebauthnChallenge(
            userID: 123, twoStepNonce: "twoStepNonce",
            success: { challengeInfo in
                expect.fulfill()
                let expectedChallengeInfo = WebauthnChallengeInfo(
                    challenge: "challenge", rpID: "wordpress.com", twoStepNonce: "two_step_nonce",
                    allowedCredentialIDs: ["credential-id, credential-id-2"])
    ...
    func testAuthenticateWebauthSignatureReturnsOauthToken() {
        ...
        let expect = expectation(description: "One callback should be invoked")
        let client = WordPressComOAuthClient(clientID: "Fake", secret: "Fake")
        client.authenticateWebauthnSignature(
            userID: 123,
